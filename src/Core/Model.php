...
class Model {
	...
	private function config()
	{
		...
		if (file_exists($filename)) {
			...
			// Monta a configuração de conexão com tipo de banco informado
			if ($conf[$enviroment]['adapter'] == 'sqlsrv')
			{
				$parameters['banco'] = [
					'configuration' => 'sqlsrv:Database',
					'host'			=> 'Server',
					'encodig'		=> 'PDO::SQLSRV_ENCODING_UTF8',
					'encod_system'	=> 'PDO::SQLSRV_ENCODING_SYSTEM'
				];
			}
			else {
				$parameters['banco'] = [
